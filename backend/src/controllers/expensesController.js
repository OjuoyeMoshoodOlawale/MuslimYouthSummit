...
export const updateDepartment = async (req, res, next) => {
  try {
    ...
    await query(
      'UPDATE departments SET name=?, description=?, head_name=?, sort_order=?, is_active=? WHERE id=?',
      [name, description || null, head_name || null, sort_order ?? 0, is_active ? 1 : 0, req.params.id]
    );
